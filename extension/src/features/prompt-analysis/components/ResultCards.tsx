...
export function ErrorState({ message, onRetry, onDismiss }: { message: string; onRetry: () => void; onDismiss: () => void }) {
  return (
    <section className="state-view error-state" role="alert">
      ...
      <div><h2 className="state-title">Analysis temporarily unavailable.</h2><p className="state-description">{message || 'Please check your connection and try again.'}</p></div>
