...
export class AnchorPositionManager {
  ...
  private resolveElements(): void {
    try {
      ...
      console.debug('[ProPar][Anchor] resolveElements for platform:', adapter.id);

      let nextAnchor = this.options.resolveAnchor?.() ?? null;
      let nextContainer = this.options.resolveContainer?.() ?? null;
      console.debug('[ProPar][Anchor] Options resolution - nextAnchor:', nextAnchor, 'nextContainer:', nextContainer);
      ...
      if (!nextAnchor) {
        ...
        console.debug('[ProPar][Anchor] Fallback check - nextComposer:', nextComposer);
        if (nextComposer) {
          nextAnchor = adapter.getAnchorElement?.(nextComposer) ?? null;
          console.debug('[ProPar][Anchor] Fallback check - nextAnchor:', nextAnchor);
          if (!nextContainer) {
            nextContainer = nextAnchor?.parentElement ?? adapter.getComposerContainer?.(nextComposer) ?? null;
            console.debug('[ProPar][Anchor] Fallback check - nextContainer:', nextContainer);
          }
        }
      }
      ...
      console.debug('[ProPar][Anchor] Final resolved - nextAnchor:', nextAnchor, 'nextContainer:', nextContainer);
      ...
      if (this.anchor && this.container) {
        ...
        console.debug('[ProPar][Anchor] Missing anchor or container. Removing host if attached.');
        if (this.hostElement && this.hostElement.parentElement) {
          this.hostElement.parentElement.removeChild(this.hostElement);
        }
      }
    } catch (err) {
      console.error('[ProPar][Anchor] Errors:', err);
    }
  }

  private ensureHost(): void {
    if (!this.container || !this.anchor) {
      console.debug('[ProPar][Anchor] ensureHost skipped due to missing anchor/container');
      return;
    }
    ...
    if (!this.hostElement) {
      console.debug('[ProPar][Anchor] Creating hostElement element');
      this.hostElement = document.createElement('div');
      this.hostElement.setAttribute('data-propar-anchor-host', '');
      ...
      this.portalElement.id = 'propar-anchor-portal';
    ...
    if (this.hostElement.parentElement !== this.container || this.hostElement.nextElementSibling !== this.anchor) {
      console.debug('[ProPar][Anchor] Inserting hostElement before anchor into container');
  ...
  private startObserving(): void {
    ...
    this.mutationObserver = new MutationObserver((mutations) => {
      const hasExternalMutation = mutations.some((m) => {
        ...
        const rootEl = document.getElementById('propar-extension-root');
    ...
    window.addEventListener('propar:platform-recover', this.handleWindowChange);
  }
  ...
  private sync(reason = 'sync'): void {
    ...
    if (this.anchor && this.container && this.hostElement) {
      ...
      if (adapter.id === 'chatgpt' || adapter.id === 'gemini' || adapter.id === 'claude' || adapter.id === 'linkedin') {
        ...
        if (adapter.id === 'chatgpt' || adapter.id === 'claude' || adapter.id === 'linkedin') {
          // Relative inline-flex layout for ChatGPT, Claude, and LinkedIn to sit naturally in flow
        ...
        // Gemini default anchor relative positioning logic
        const anchorRect = this.anchor.getBoundingClientRect();
        const relativeTop = anchorRect.top - containerRect.top;
        const relativeLeft = anchorRect.left - containerRect.left;

        // Spacing: 14px for Gemini
        const spacing = 14;
        const hostTop = relativeTop + (anchorRect.height - 36) / 2;
        const hostLeft = relativeLeft - 36 - spacing;

        const nextTop = `${hostTop}px`;
        const nextLeft = `${hostLeft}px`;
        if (this.hostElement.style.top !== nextTop) {
          this.hostElement.style.top = nextTop;
        }
        if (this.hostElement.style.left !== nextLeft) {
          this.hostElement.style.left = nextLeft;
        }
        this.hostElement.style.right = 'auto';
        this.hostElement.style.bottom = 'auto';
        
        // Ensure margins are cleared for absolute positioning
        this.hostElement.style.marginLeft = '';
        this.hostElement.style.marginRight = '';
        this.hostElement.style.alignSelf = '';
      }
